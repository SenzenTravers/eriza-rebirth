...
class Coureur(commands.Cog):
    ...
    async def course(self, ctx, *arg):
        ...
        """ 
        error_already_course = "Une course est déjà en cours !"
        ...
        if self.sprint:
            ...
            if start_time.minute < 10:
                await ctx.channel.send(f"Le sprint commencera à {start_time.hour}h0{start_time.minute} pour {duration} minutes.")
            else:
                await ctx.channel.send(f"Le sprint commencera à {start_time.hour}h{start_time.minute} pour {duration} minutes.")
            ...
            if self.runners == {}:
                ...
                await ctx.channel.send(f"Le sprint COMMENCE ! Vous avez {duration} minutes !")
                await asyncio.sleep(sprint_data[1])

                mentions = ", ".join([dude.mention for dude in self.runners.keys()])
                await ctx.channel.send(f"{mentions} ABOULEZ LES MOTS ! Vous avez trois (3) minutes.")
                await asyncio.sleep(180)
                finished_list = self.winners_list()

                await ctx.channel.send(f"C'EEEEEEST *FINI* ! Nos protagonistes : {finished_list}")
                self.sprint = False
                self.coureurs = {}
                self.enders = {}
    ...
    def winners_list(self):
        ...
        for user_results in sorted_users:
            ...
            if written_words > 0:
                sorted_users = [f"""
:star2: {item[0].mention}: {item[1]} mots, dont {item[2]} nouveaux mots !""" for item in cleaned_users]
